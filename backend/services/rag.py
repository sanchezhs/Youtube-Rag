...
class RAGService:
    ...
    def ask(
        ...
        *,
        ...
        if intent == "METADATA":
            ...
            # Poll every 0.2s, timeout after 10s
    ...
    def _wait_for_embedding(self, task_id: UUID) -> list[float]:
        ...
        while True:
            row = self.db.execute(
                select(PipelineTask.status, PipelineTask.result).where(PipelineTask.id == task_id)
            ).one()

            if row.status.upper() == "COMPLETED" and row.result:
                return row.result
            
            if row.status == "FAILED":
                raise Exception("Embedding task failed in worker.")
